...
FROM=$((KNOWN_TS - 5))
TO=$((KNOWN_TS + 5))
run_moosh search:timestamp -p "$MOODLE_PATH" --from $FROM --to $TO -o csv --limit 5
echo "$OUT" | head -5
...
run_moosh search:timestamp -p "$MOODLE_PATH" $KNOWN_TS -o json --limit 2
echo "$OUT" | head -10
...
run_moosh search:timestamp -p "$MOODLE_PATH" $KNOWN_TS --limit 3
...
run_moosh search:timestamp -p "$MOODLE_PATH" $KNOWN_TS -o csv --limit 2
...
run_moosh search:timestamp -p "$MOODLE_PATH" 9999999999 -o csv
...
run_moosh search:timestamp -p "$MOODLE_PATH"
EXIT_CODE=$?
assert_exit_code "Exit code 1 for no args" 1 "$EXIT_CODE"
echo ""

echo "--- Test: Both timestamp and range ---"
run_moosh search:timestamp -p "$MOODLE_PATH" --from 100 --to 200 $KNOWN_TS
EXIT_CODE=$?
...
run_moosh search:timestamp -p "$MOODLE_PATH" --from 100
EXIT_CODE=$?
...
run_moosh search:timestamp -p "$MOODLE_PATH" --help
