...
run_moosh sql:select -p "$MOODLE_PATH" "SELECT id, username, email FROM {user} WHERE id <= 3" -o csv
...
run_moosh sql:select -p "$MOODLE_PATH" "SELECT id, username FROM {user} WHERE username = 'admin'"
...
run_moosh sql:select -p "$MOODLE_PATH" "SELECT id, shortname FROM {course} WHERE id = 2" -o json
...
run_moosh sql:select -p "$MOODLE_PATH" "SELECT id FROM {user}" --limit 3 -o csv
...
run_moosh sql:select -p "$MOODLE_PATH" "SELECT COUNT(*) AS total FROM {course} WHERE id > 1" -o csv
assert_output_contains "Header has total" "total" "$OUT"
assert_output_contains "17 courses" "17" "$OUT"
echo ""

# ── Join query ────────────────────────────────────────────────────

echo "--- Test: Join query ---"
run_moosh sql:select -p "$MOODLE_PATH" "SELECT u.username, e.enrol FROM {user_enrolments} ue JOIN {enrol} e ON e.id = ue.enrolid JOIN {user} u ON u.id = ue.userid WHERE e.courseid = 2 AND u.username = 'student01'" -o csv
...
run_moosh sql:select -p "$MOODLE_PATH" "SELECT id FROM {user} WHERE username = 'nonexistent'" -o csv
EXIT_CODE=$?
...
run_moosh sql:select -p "$MOODLE_PATH" "DELETE FROM {user}"
EXIT_CODE=$?
...
run_moosh sql:select -p "$MOODLE_PATH" "UPDATE {user} SET username = 'x'"
EXIT_CODE=$?
...
run_moosh sql:select -p "$MOODLE_PATH" "INSERT INTO {user} (username) VALUES ('x')"
EXIT_CODE=$?
...
run_moosh sql:select -p "$MOODLE_PATH" "SELECT * FROM {nonexistent_table}"
EXIT_CODE=$?
...
run_moosh sql:select -p "$MOODLE_PATH" --help
