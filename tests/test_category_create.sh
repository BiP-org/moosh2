...
run_moosh category:create -p "$MOODLE_PATH" "New Category"
...
run_moosh category:list -p "$MOODLE_PATH" --sql "cc.name = 'New Category'" -o csv
VERIFY="$OUT"
...
run_moosh category:create -p "$MOODLE_PATH" --run "New Category" -o csv
...
run_moosh category:create -p "$MOODLE_PATH" --run "Cat A" "Cat B" "Cat C" -o csv
...
run_moosh category:create -p "$MOODLE_PATH" --run --parent 2 "Linear Algebra" -o csv
...
run_moosh category:list -p "$MOODLE_PATH" --sql "cc.name = 'Linear Algebra'" -o csv
VERIFY="$OUT"
...
run_moosh category:create -p "$MOODLE_PATH" --run \
...
run_moosh category:create -p "$MOODLE_PATH" --run --visible 0 "Hidden Category" -o csv
...
run_moosh category:create -p "$MOODLE_PATH" --run "JSON Cat" -o json
...
run_moosh category:create -p "$MOODLE_PATH" --help
