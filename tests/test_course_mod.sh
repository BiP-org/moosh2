...
echo "--- Test: Dry run ---"
run_moosh course:mod 2 --fullname "New Name" -p "$MOODLE_PATH"
EC=$?
...
run_moosh course:mod 2 --fullname "Updated Algebra" -p "$MOODLE_PATH" --run
EC=$?
...
run_moosh course:mod 2 --shortname "algebra_updated" -p "$MOODLE_PATH" --run -o csv
EC=$?
...
run_moosh course:mod 2 --visible 0 -p "$MOODLE_PATH" --run -o csv
EC=$?
assert_exit_code "Visible exit code 0" 0 $EC
# Verify it's hidden
run_moosh course:mod 2 --visible 1 -p "$MOODLE_PATH" --run -o csv
assert_exit_code "Restore visible exit code 0" 0 $EC
echo ""

echo "--- Test: Change idnumber ---"
run_moosh course:mod 2 --idnumber "ALG001" -p "$MOODLE_PATH" --run
EC=$?
assert_exit_code "Idnumber exit code 0" 0 $EC
echo ""

echo "--- Test: Change format ---"
run_moosh course:mod 2 --format weeks -p "$MOODLE_PATH" --run -o csv
EC=$?
...
run_moosh course:mod 2 --startdate "2025-09-01" -p "$MOODLE_PATH" --run -o csv
EC=$?
...
run_moosh course:mod 2 --enddate "2026-06-30" -p "$MOODLE_PATH" --run -o csv
EC=$?
...
run_moosh course:mod 2 --category 3 -p "$MOODLE_PATH" --run -o csv
EC=$?
...
run_moosh course:mod 2 --guest 1 -p "$MOODLE_PATH" --run
EC=$?
...
run_moosh course:mod 2 --selfenrol 1 -p "$MOODLE_PATH" --run
EC=$?
assert_exit_code "Selfenrol enable exit code 0" 0 $EC
echo ""

echo "--- Test: Multiple changes at once ---"
run_moosh course:mod 2 --fullname "Multi Change" --visible 1 --lang en -p "$MOODLE_PATH" --run
EC=$?
...
run_moosh course:mod 2 --startdate "not-a-date" -p "$MOODLE_PATH" --run
EC=$?
...
run_moosh course:mod 999 --fullname "Test" -p "$MOODLE_PATH" --run
EC=$?
...
run_moosh course:mod 2 --category 999 -p "$MOODLE_PATH" --run
EC=$?
...
run_moosh course:mod 2 -p "$MOODLE_PATH"
EC=$?
...
run_moosh course:mod 2 --summary "Test summary" -p "$MOODLE_PATH" --run -o json
assert_output_contains "JSON has shortname" '"shortname"' "$OUT"
echo ""

echo "--- Test: course:mod help ---"
run_moosh course:mod -p "$MOODLE_PATH" --help
