...
run_moosh sql:run -p "$MOODLE_PATH" "SELECT id, username FROM {user} WHERE username='admin'"
...
run_moosh sql:run -p "$MOODLE_PATH" "SELECT id, username FROM {user} WHERE username='admin'" -o csv
...
run_moosh sql:run -p "$MOODLE_PATH" "SELECT id, username FROM {user} WHERE username='admin'" -o json
assert_output_contains "JSON has username" '"username": "admin"' "$OUT"
echo ""

echo "--- Test: sql:run SELECT with --limit ---"
run_moosh sql:run -p "$MOODLE_PATH" "SELECT id FROM {user}" --limit=3 -o csv
...
run_moosh sql:run -p "$MOODLE_PATH" "SELECT id FROM {user} WHERE username='nonexistent_user_xyz'" -o csv
assert_output_not_contains "No data rows for empty result" "nonexistent_user_xyz" "$OUT"
echo ""

echo "--- Test: sql:run SELECT invalid SQL ---"
run_moosh sql:run -p "$MOODLE_PATH" "SELECT * FROM {nonexistent_table_xyz}"
EXIT_CODE=$?
...
run_moosh sql:run -p "$MOODLE_PATH" "SELECT id, shortname FROM {course} WHERE id > 1" --limit=5 -o csv
...
run_moosh sql:run -p "$MOODLE_PATH" "UPDATE {user} SET city='TestCity' WHERE username='admin'"
...
run_moosh sql:run -p "$MOODLE_PATH" "UPDATE {user} SET city='TestCity' WHERE username='admin'" --run
...
run_moosh sql:run -p "$MOODLE_PATH" "DELETE FROM {nonexistent_table_xyz}" --run
EXIT_CODE=$?
...
run_moosh sql:run -p "$MOODLE_PATH" --help
...
run_moosh sql:dump -p "$MOODLE_PATH" --file="$TMPDIR/dump.sql" --tables=config
...
run_moosh sql:dump -p "$MOODLE_PATH" --gzip --file="$TMPDIR/dump.sql.gz" --tables=config
...
run_moosh sql:dump -p "$MOODLE_PATH" --tables=user --file="$TMPDIR/dump_user.sql"
...
run_moosh sql:dump -p "$MOODLE_PATH" --help
...
run_moosh sql:cli -p "$MOODLE_PATH" --help
