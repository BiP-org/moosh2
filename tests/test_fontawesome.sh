...
run_moosh fontawesome:maplist -p "$MOODLE_PATH" -o csv
...
run_moosh fontawesome:maplist -p "$MOODLE_PATH" search -o csv
...
run_moosh fontawesome:maplist -p "$MOODLE_PATH" calendar -o csv
assert_output_contains "Finds calendar icon" "calendar" "$OUT"
echo ""

echo "--- Test: Component filter ---"
run_moosh fontawesome:maplist -p "$MOODLE_PATH" --component theme -o csv
echo "$OUT" | head -5
...
run_moosh fontawesome:maplist -p "$MOODLE_PATH" --component core search -o csv
...
run_moosh fontawesome:maplist -p "$MOODLE_PATH" xyznonexistent123 -o csv
LINE_COUNT=$(echo "$OUT" | wc -l)
assert_output_contains "Only header for no results" "1" "$LINE_COUNT"
echo ""

echo "--- Test: JSON output ---"
run_moosh fontawesome:maplist -p "$MOODLE_PATH" search -o json
echo "$OUT" | head -10
...
run_moosh fontawesome:maplist -p "$MOODLE_PATH" search
...
echo "--- Test: Help ---"
run_moosh fontawesome:maplist -p "$MOODLE_PATH" --help
...
echo "--- Test: Dry run ---"
run_moosh fontawesome:refresh-cache -p "$MOODLE_PATH"
assert_output_contains "Shows dry run" "Dry run" "$OUT"
echo ""

echo "--- Test: Refresh cache ---"
run_moosh fontawesome:refresh-cache -p "$MOODLE_PATH" --run
assert_output_contains "Shows refreshed" "refreshed" "$OUT"
assert_output_contains "Shows mappings count" "mappings" "$OUT"
echo ""

echo "--- Test: Help ---"
run_moosh fontawesome:refresh-cache -p "$MOODLE_PATH" --help
