...
run_moosh auth:list -p "$MOODLE_PATH" -o csv
...
run_moosh auth:list -p "$MOODLE_PATH" --enabled-only -o csv
...
run_moosh auth:list -p "$MOODLE_PATH" -o json
...
run_moosh auth:info -p "$MOODLE_PATH" manual -o json
echo "$OUT" | head -15
...
run_moosh auth:info -p "$MOODLE_PATH" email -o json
...
run_moosh auth:info -p "$MOODLE_PATH" ldap -o json
assert_output_contains "LDAP disabled" '"Enabled": "no"' "$OUT"
echo ""

echo "--- Test: Table output ---"
run_moosh auth:info -p "$MOODLE_PATH" manual
...
run_moosh auth:info -p "$MOODLE_PATH" nonexistent
EXIT_CODE=$?
...
echo "--- Test: Help ---"
run_moosh auth:info -p "$MOODLE_PATH" --help
...
echo "--- Test: Dry run ---"
run_moosh auth:mod -p "$MOODLE_PATH" enable ldap
assert_output_contains "Shows dry run" "Dry run" "$OUT"
assert_output_contains "Shows plugin" "ldap" "$OUT"
echo ""

echo "--- Test: Enable ldap ---"
run_moosh auth:mod -p "$MOODLE_PATH" --run enable ldap
...
run_moosh auth:mod -p "$MOODLE_PATH" --run up ldap
...
echo "--- Test: Move down ---"
run_moosh auth:mod -p "$MOODLE_PATH" --run down ldap
assert_output_contains "Action down applied" "down" "$OUT"
echo ""

echo "--- Test: Disable ldap ---"
run_moosh auth:mod -p "$MOODLE_PATH" --run disable ldap
...
run_moosh auth:mod -p "$MOODLE_PATH" --run disable manual
EXIT_CODE=$?
...
run_moosh auth:mod -p "$MOODLE_PATH" --run invalid ldap
EXIT_CODE=$?
...
echo "--- Test: Invalid plugin ---"
run_moosh auth:mod -p "$MOODLE_PATH" --run enable nonexistent
EXIT_CODE=$?
assert_exit_code "Exit code 1 for invalid plugin" 1 "$EXIT_CODE"
...
echo "--- Test: Help ---"
run_moosh auth:mod -p "$MOODLE_PATH" --help
