...
echo "--- Test: Dry run ---"
run_moosh badge:create -p "$MOODLE_PATH" "Dry Run Badge"
...
run_moosh badge:create -p "$MOODLE_PATH" --run -d "A site-level badge" "Site Badge" -o csv
...
run_moosh badge:create -p "$MOODLE_PATH" --run --course 2 -d "Course-level badge" "Course Badge" -o csv
...
run_moosh badge:create -p "$MOODLE_PATH" --run "JSON Badge" -o json
...
run_moosh badge:create -p "$MOODLE_PATH" --run --course 99999 "Bad Badge"
EXIT_CODE=$?
...
echo "--- Test: Help ---"
run_moosh badge:create -p "$MOODLE_PATH" --help
...
run_moosh badge:info -p "$MOODLE_PATH" $SITE_BADGE_ID -o json
echo "$OUT" | head -15
...
run_moosh badge:info -p "$MOODLE_PATH" $COURSE_BADGE_ID -o json
...
run_moosh badge:info -p "$MOODLE_PATH" $SITE_BADGE_ID
...
run_moosh badge:info -p "$MOODLE_PATH" 99999
EXIT_CODE=$?
...
echo "--- Test: Help ---"
run_moosh badge:info -p "$MOODLE_PATH" --help
...
echo "--- Test: Dry run ---"
run_moosh badge:mod -p "$MOODLE_PATH" --name "New Name" $SITE_BADGE_ID
assert_output_contains "Shows dry run" "Dry run" "$OUT"
assert_output_contains "Shows name change" "name:" "$OUT"
echo ""

echo "--- Test: Rename badge ---"
run_moosh badge:mod -p "$MOODLE_PATH" --run --name "Renamed Badge" $SITE_BADGE_ID -o csv
assert_output_contains "Renamed" "Renamed Badge" "$OUT"
echo ""

echo "--- Test: Update description ---"
run_moosh badge:mod -p "$MOODLE_PATH" --run -d "New description" $SITE_BADGE_ID -o csv
...
run_moosh badge:mod -p "$MOODLE_PATH" --run $SITE_BADGE_ID
EXIT_CODE=$?
...
run_moosh badge:mod -p "$MOODLE_PATH" --run --status invalid $SITE_BADGE_ID
EXIT_CODE=$?
...
echo "--- Test: Help ---"
run_moosh badge:mod -p "$MOODLE_PATH" --help
...
echo "--- Test: Dry run ---"
run_moosh badge:delete -p "$MOODLE_PATH" $COURSE_BADGE_ID
assert_output_contains "Shows dry run" "Dry run" "$OUT"
assert_output_contains "Shows badge ID" "ID=$COURSE_BADGE_ID" "$OUT"
echo ""

echo "--- Test: Delete single badge ---"
run_moosh badge:delete -p "$MOODLE_PATH" --run $COURSE_BADGE_ID
...
run_moosh badge:delete -p "$MOODLE_PATH" --run $JSON_BADGE_ID
assert_output_contains "First deleted" "Deleted" "$OUT"
echo ""

echo "--- Test: Invalid badge ID ---"
run_moosh badge:delete -p "$MOODLE_PATH" --run 99999
EXIT_CODE=$?
assert_exit_code "Exit code 1 for invalid badge" 1 "$EXIT_CODE"
assert_output_contains "Not found error" "not found" "$OUT"
echo ""
...
echo "--- Test: Help ---"
run_moosh badge:delete -p "$MOODLE_PATH" --help
